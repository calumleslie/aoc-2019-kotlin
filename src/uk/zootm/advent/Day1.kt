...
fun main() {
    FileInputStream(File("inputs/day1.1.txt")).use {
        val sum = it.bufferedReader().lines()
            .mapToInt { fuelFor(Integer.parseInt(it)) }
            .sum()
    ...
    FileInputStream(File("inputs/day1.1.txt")).use {
        val sum = it.bufferedReader().lines()
            .mapToInt { totalFuelFor(Integer.parseInt(it)) }
            .sum()
...
fun totalFuelFor(mass: Int): Int {
    var total = 0
    var unhandled = mass
    while (true) {
        val additionalFuel = fuelFor(unhandled)
        if (additionalFuel <= 0) {
            return total
        } else {
            total += additionalFuel
            unhandled = additionalFuel
        }
    }
}

fun fuelFor(mass: Int): Int = (mass / 3) - 2